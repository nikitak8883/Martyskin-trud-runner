/**
 * Pure M03.3A development-diagnostics contract.
 *
 * Runtime wiring is intentionally deferred to M03.3C. The default remains
 * fail-closed disabled so merely constructing the log cannot enable release
 * diagnostics.
 */
...
export type DevJsonValue =
    | DevScalar
    | readonly DevJsonValue[]
    | Readonly<{ [key: string]: DevJsonValue }>;
...
export function isDevEventCode(value: unknown): value is DevEventCode {
    return typeof value === 'string'
        && (DEV_EVENT_CODES as readonly string[]).indexOf(value) >= 0;
}
