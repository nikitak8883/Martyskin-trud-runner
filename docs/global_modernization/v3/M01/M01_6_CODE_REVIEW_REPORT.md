...
The first remote Windows/Linux matrix run is post-push evidence. Any failed job blocks continuation of the published source branch and must be fixed by rerunning the same command, not by weakening or conditionally skipping a step.
