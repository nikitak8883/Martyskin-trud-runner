...
Продолжать `M04-C-FAMILIES` ровно одним следующим measured child: до мутации зафиксировать multi-descriptor screen-coverage contract для `ui_shared_core`. Пакетная перепаковка, изменение dynamic-atlas policy, physical-device QA и release остаются запрещены до своих отдельных gates.
