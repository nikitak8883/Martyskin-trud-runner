...
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $ScreenshotPath), (Split-Path -Parent $BrowserLogPath), (Split-Path -Parent $ProbePath) | Out-Null
Remove-Item -LiteralPath $ScreenshotPath, $BrowserLogPath, $ProbePath -Force -ErrorAction SilentlyContinue
...
try {
    ...
    while ((Get-Date) -lt $readyDeadline) {
        ...
        if (Test-Path -LiteralPath $BrowserLogPath) {
            ...
            if ($text -match "MTR_RUNTIME_CORE_READY") {
                $ready = $true
                break
            }
        }
