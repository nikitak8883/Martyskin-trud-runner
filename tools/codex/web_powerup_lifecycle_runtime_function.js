async (page) => {
    ...
    url.searchParams.set('mtr_qa_powerups', '1');
    url.searchParams.set('mtr_cache_bust', `${Date.now()}_powerup_lifecycle`);

    const deadline = Date.now() + 45000;
    await page.goto(url.href, { waitUntil: 'load', timeout: 30000 });
    while (Date.now() < deadline) {
        const messages = consoleEvents.map((event) => event.text);
        if (messages.some((text) => /MTR_POWERUP_QA_(?:READY|FAIL)/i.test(text))) break;
        await page.waitForTimeout(100);
    }
    await page.waitForTimeout(500);

    const messages = consoleEvents.map((event) => event.text);
    const expectedPhases = 'spawned>collected>activated>expired>cleaned';
    const readyMessages = messages.filter((text) => /MTR_POWERUP_QA_READY/i.test(text));
    const failureMessages = messages.filter((text) => /MTR_POWERUP_QA_FAIL/i.test(text));
    ...
    const exactReady = readyMessages.some((text) => (
        ...
        && text.includes(`phases=${expectedPhases}`)
        && /stale=rejected\b/i.test(text)
    ...
    return {
        schema: 'mtr.web_powerup_lifecycle_runtime.v1',
