...
param(
    ...
    [string]$OutputPath = 'temp\m03-7a-android-runtime-ownership.json',
...
do {
    ...
    if ($logcat -match 'MTR_OWNERSHIP_QA_(?:READY|FAIL)') { break }
...
$readyPattern = 'MTR_OWNERSHIP_QA_READY checks=8/8 listeners=8 sessionCancelled=1 componentSurvived=1 uiTransition=1 state=playing'
$readyMatches = [regex]::Matches($logcat, $readyPattern)
$failureMatches = [regex]::Matches($logcat, 'MTR_OWNERSHIP_QA_FAIL')
...
$result = [ordered]@{
    schema = 'mtr.android_runtime_ownership.v1'
